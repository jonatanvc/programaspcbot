...
async def scrape_filecr(url_apps="https://filecr.com/ms-windows/"):
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            page = await context.new_page()
            try:
                await page.goto(url_apps, wait_until="domcontentloaded", timeout=40000)
            except Exception as e:
                from config import logger
                logger.warning(f"FCR-Scraper: Timeout parcial al cargar {url_apps}. Se intentará procesar el contenido disponible. Detalle: {e}")
