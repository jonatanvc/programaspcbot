...
async def obtener_enlace_dinamico(context, url_post, titulo="Software"):
    ...
    try:
        ...
        if await boton_descarga.count() > 0:
            ...
            # Intentar interceptar la descarga automática o la navegación a la página intermedia
            enlace_final = None
            try:
                import asyncio
                # Primero intentamos atrapar una navegación a una página intermedia (ej. apkteal.com/xyz)
                ...
                # Si no hubo navegación, intentamos interceptar un archivo
                try:
                    import asyncio
            ...
            if not enlace_final or enlace_final == url_post:
                # Extraer el enlace usando JavaScript para buscar href
                enlace_final = await page.evaluate('''() => {
                    ...
                    for (let el of allElements) {
                        ...
                        if (url && !url.includes('filecr.com') && !url.startsWith('/') && !url.includes('t.me') && !url.includes('twitter') && !url.includes('youtube') && !url.includes('facebook') && !url.includes('reddit') && !url.includes('pinterest') && !url.includes('vk.com') && !url.includes('instagram') && !url.includes('trustpilot')) {
                            // Devolver el primero que sea válido para no tomar los links del footer
                            return url;
                        }
                    }
                    return null;
