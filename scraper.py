        ...
        try:
            ...
            metadata = await page.evaluate('''() => {
                ...
                if (cats.length > 0) {
                    ...
                    let meta_tags = document.querySelectorAll('meta[property="article:tag"]');
                    let m_cats = [];
                    meta_tags.forEach(m => m_cats.push(m.getAttribute('content')));
                    if (m_cats.length > 0) cat = m_cats.join(', ');
                }
                ...
                let reqs = '';
                // The requirements are usually under a heading like "System Requirements" or inside __NEXT_DATA__
                ...
                if (next_data) {
                    try {
                        ...
                        if (post.requirements) {
                            // Strip HTML tags from requirements
                    ...
                    h3s.forEach(h => {
                        if (h.innerText.includes('System Requirements') || h.innerText.includes('Technical Details')) {
                            ...
                            if (next && next.tagName === 'UL') {
                                ...
                                let r_arr = [];
                                lis.forEach(li => r_arr.push('• ' + li.innerText.trim()));
                                reqs = r_arr.join('\\n');
                            }
                        }
                    });
                ...
                return {title, desc, cat, version, img, date, reqs};
            }''')
