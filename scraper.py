...
async def download_and_crop_image(url: str) -> str:
    ...
    try:
        ...
        def process_img(data):
            ...
            # GetIntoPC pone una gran marca de agua amarilla en la mitad inferior.
            # Recortaremos manteniendo el 60% superior de la imagen.
...
async def obtener_enlace_dinamico_getintopc(context, url_post, titulo="Software"):
    ...
    try:
        ...
        metadata = await page.evaluate('''() => {
            ...
            let img = document.querySelector('.post-content img');
            ...
            for(let i=0; i<p_tags.length; i++) {
                ...
                // Ignorar parrafos de spam o muy cortos, o que dicen "Click on below button"
                if(text.length > 40 && !text.toLowerCase().includes('download') && !text.toLowerCase().includes('password')) {
                    data.descripcion += text + "\\n\\n";
                    if(data.descripcion.length > 500) break; // Limitar longitud
                }
            }
            ...
            if(req_header) {
                ...
                if(ul) {
                    data.requisitos = ul.innerText.trim();
                } else {
                    // a veces esta en el mismo parrafo
                    data.requisitos = req_header.innerText.replace(/requirements/i, '').trim();
                }
            }
        ...
        return {
            ...
            'titulo': titulo,
            'version': version_ext,
            'categoria': metadata.get('categoria', 'Software'),
