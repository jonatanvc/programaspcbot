...
async def scrape_filecr(url_apps="https://filecr.com/ms-windows/"):
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            # Nuevo enfoque Anti-Cloudflare: Leer el Sitemap XML directamente para obtener URLs 100% al azar y válidas
            ...
            try:
                ...
                if slugs:
                    ...
                    posts_a_procesar = []
                    for slug in slugs:
                        # Limpiamos el slug para hacerlo título
            ...
            logger.info(f"Se detectaron {len(posts_a_procesar)} programas en el feed (vía Sitemap). Extrayendo metadatos y enlaces finales...")
            ...
            for titulo, url_post in posts_a_procesar:
                ...
                data = await obtener_enlace_dinamico(context, url_post, titulo)
