...
async def obtener_enlace_dinamico(context, url_post):
    ...
    try:
        ...
        if await boton_descarga.count() > 0:
            ...
            cat_tag = page.locator(".breadcrumbs_wrap__qBADP li:last-child").first
            if await cat_tag.count() > 0:
                categoria_real = await cat_tag.inner_text()
            ...
            if await date_tag.count() > 0:
                ...
                if fecha_raw: fecha_actualizacion = fecha_raw.split("T")[0].split(" ")[0]
