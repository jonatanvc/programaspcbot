                ...
                try:
                    ...
                    if "filecr.com" not in href.lower():
                        if href.startswith("magnet:") or href.endswith((".zip", ".rar", ".iso", ".7z", ".exe")):
                            enlace_final = href
                            break
                        if "?ref=" in href.lower() or "/download/" in href.lower():
                            enlace_final = href
                            break
                            
                        # Guardar en fallback si no es red social
                        if not any(x in href.lower() for x in ['t.me', 'twitter', 'facebook', 'youtube', 'instagram', 'reddit', 'trustpilot', 'pinterest', 'vk.com']):
                            if href not in posibles_enlaces:
                                posibles_enlaces.append(href)
                except Exception:
                    continue
                    
            if not enlace_final and posibles_enlaces:
                from config import logger
                logger.warning(f"FCR-Scraper: Fallback activado. Usando primer enlace de: {posibles_enlaces}")
                enlace_final = posibles_enlaces[-1] # Usually the download link is added at the end of the DOM
