...
async def scrape_search(query, require_os=False):
    ...
    try:
        ...
        logger.error(f"Error en scrape_search: {e}")
...
async def scrape_search_getintopc(query, require_os=False):
    ...
    logger.info(f"GetIntoPC-Scraper: Buscando '{query}' en GetIntoPC...")
    try:
        async with async_playwright() as p:
            browser = await p.chromium.launch(headless=True)
            context = await browser.new_context(
            ...
            items = await page.evaluate('''() => {
                ...
                for(let p of posts) {
                    ...
                    let descNode = p.querySelector('.post-content p');
            ...
            for item in items:
                ...
                logger.info(f"GetIntoPC-Scraper: Match encontrado -> {item['titulo']}")
                return item
                
            return None
            
    except Exception as e:
        logger.error(f"GetIntoPC-Scraper Error de búsqueda: {e}")
        return None
...
async def obtener_enlace_dinamico_getintopc(context, url_post, titulo="Software"):
    ...
    try:
        ...
        metadata = await page.evaluate('''() => {
            let data = { requisitos: "", descripcion: "", publisher: "GetIntoPC" };
            let p_tags = document.querySelectorAll('.post-content p');
            for(let i=0; i<Math.min(4, p_tags.length); i++) {
                if(p_tags[i].innerText.length > 40 && !p_tags[i].innerText.toLowerCase().includes('download')) {
                    data.descripcion += p_tags[i].innerText.trim() + "\\n\\n";
                }
            }
            
            let req_header = Array.from(document.querySelectorAll('h3, h2')).find(el => el.innerText.toLowerCase().includes('requirements'));
            if(req_header) {
                let ul = req_header.nextElementSibling;
                while(ul && ul.tagName !== 'UL' && ul.tagName !== 'P') {
                    ul = ul.nextElementSibling;
                }
                if(ul) data.requisitos = ul.innerText.trim();
            }
            return data;
        }''')
        
        enlace_final = None
        async def intercept_download(route, request):
            nonlocal enlace_final
            if request.url.endswith('.zip') or request.url.endswith('.rar') or request.url.endswith('.iso'):
                enlace_final = request.url
                await route.abort()
            else:
                await route.continue_()
                
        await page.route("**/*", intercept_download)
        
        btn_download = page.locator("input[type='submit'][value*='Download'], button:has-text('Download'), a:has-text('Download Full Setup')").first
        if await btn_download.count() > 0:
            async with page.expect_popup() as popup_info:
                await btn_download.click()
            popup = await popup_info.value
            await popup.route("**/*", intercept_download)
            
            for _ in range(25):
                if enlace_final: break
                await asyncio.sleep(1)
            await popup.close()
            
        if not enlace_final:
            # Fallback a buscar el enlace manualmente si no saltó como popup
            pass
