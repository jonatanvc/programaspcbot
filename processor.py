...
async def download_file(url, file_name):
    ...
    logger.warning("Fallo Aria2c después de todos los intentos. Cayendo a fallback nativo de Python (urllib)...")
    try:
        ...
        if os.path.getsize(file_path) > 10240:
            ...
            try:
                with open(file_path, 'rb') as f:
                    header = f.read(1024).decode('utf-8', errors='ignore').lower()
                    if "<html" in header or "<!doctype html" in header or "<body" in header:
                        logger.error("Fallback Python descargó una página HTML falsa (Cloudflare/Error). Descartando...")
                        os.remove(file_path)
                        return None
            except Exception:
                pass
            
            logger.info(f"Fallback Python exitoso: {file_path}")
            return file_path
        else:
            logger.error("Fallback Python descargó un archivo demasiado pequeño (posible bloqueo).")
            os.remove(file_path)
    except Exception as py_err:
        logger.error(f"Fallback nativo de Python también falló: {py_err}")
