...
async def download_file(url, file_name):
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            cmd = f'aria2c -x 4 -s 4 -k 1M --check-certificate=false -U "Mozilla/5.0 (Windows NT 10.0; Win64; x64)" --auto-file-renaming=false --timeout=60 --max-tries=3 -d "{DOWNLOAD_DIR}" -o "{file_name}" "{url}"'
