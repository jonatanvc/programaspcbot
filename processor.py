    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            try:
                ...
                # Limpiar residuos por si el archivo quedó corrupto
                try:
                    if os.path.exists(file_path): os.remove(file_path)
                    if os.path.exists(f"{file_path}.aria2"): os.remove(f"{file_path}.aria2")
                except: pass
                continue
...
async def split_file_if_needed(file_path):
    ...
    try:
        ...
        try:
            ...
            # Limpiar archivo parcialmente creado por 7z
            try:
...
def startup_cleanup_folders():
    ...
    for folder in folders_to_clean:
        if os.path.exists(folder):
            try:
                # Borrar el contenido de la carpeta en lugar de la carpeta (evita Error 16: Device or resource busy en Docker volumes)
                ...
                logger.info(f"Limpieza de inicio: Contenido de {folder} purgado completamente.")
            except Exception as e:
