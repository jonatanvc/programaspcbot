    ...
    while True:
        try:
            ...
            for prog in resultados:
                if not await program_exists(db_pool, prog['url_origen']):
                    await insert_program(db_pool, prog)
                    nuevos += 1
...
async def worker_queue_processor():
    while True:
        prog = await get_pending_programs(db_pool, limit=1)
        if not prog:
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "menu_main":
        ...
        if user_id != ADMIN_ID: return
        errores = await get_error_programs(db_pool, limit=5)
        if not errores:
