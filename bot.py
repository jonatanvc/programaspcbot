...
async def scheduled_scraper_task():
    while True:
        try:
            ...
            # Rotar entre categorías para obtener variedad (ISOs, Antivirus, etc)
            import random
            categorias = [
                "https://filecr.com/ms-windows/",
                "https://filecr.com/windows/operating-systems/",
                "https://filecr.com/windows/video-editors/",
                "https://filecr.com/windows/antivirus/",
                "https://filecr.com/windows/office-pdf/",
                "https://filecr.com/windows/3d-design/",
                "https://filecr.com/windows/development/",
                "https://filecr.com/windows/audio-editors/",
                "https://filecr.com/mac/"
            ]
            url_a_scrapear = random.choice(categorias)
...
async def process_single_program(prog):
    async with download_semaphore:
        ...
        import urllib.parse
        original_filename = urllib.parse.unquote(prog['url_origen'].split('/')[-1].split('?')[0].split('#')[0])
        if not original_filename or len(original_filename) < 3:
            original_filename = f"{prog['titulo'].replace(' ', '_')}.zip"
            
        file_path = await download_file(prog['url_origen'], original_filename)
        if not file_path:
            await update_status(db_pool, prog['id'], 'Error')
            # Limpieza forzosa de archivos parciales y .aria2
    ...
    # Fuera del semáforo para no bloquear descargas durante el upload (o podemos dejarlo dentro si el upload consume muchos recursos, pero el upload en Local API es rápido)
    file_ids = []
    ...
    if len(file_ids) == len(part_paths):
        ...
        if PUBLIC_CHANNEL_ID != 0:
            async with publish_lock:
                version_str = prog.get('version')
                if version_str and version_str.lower() != "latest" and version_str != "Desconocida":
                    title_line = f"🌟 {prog['titulo']} - {version_str}"
                else:
                    title_line = f"🌟 {prog['titulo']}"
                    
                texto = f"{title_line}\n\n"
                texto += f"🎲 Categoría: #{prog.get('categoria', 'Software').replace(' ', '_').replace('/', '_').replace('-', '_')}\n"
                texto += f"📅 Actualización: {prog.get('fecha_actualizacion', 'Reciente')}\n"
                texto += f"👨‍💻 Sistema: Windows/Mac\n\n"
                
                desc = prog.get('descripcion', 'Utilidad distribuida para Windows.')
                if len(desc) > 350:
                    desc = desc[:347] + "..."
                texto += f"Descripción:\n{desc}\n\n"
                texto += f"💾 Tamaño: {size_str}   📚 Archivos: {archivos_count}\n\n"
                texto += f"⬇️ Pulsa el botón inferior para descargar de forma segura."
                ...
                logger.info("Esperando 12 minutos para evitar flood en el canal publico (Modo Gotero)...")
...
async def worker_queue_processor():
    while True:
        progs = await get_pending_programs(db_pool)
        if not progs:
        ...
        for prog in progs:
            ...
            asyncio.create_task(process_single_program(prog))
            
            # Pequeña pausa para no encolar cientos de tareas de golpe y colapsar el event loop
