...
async def worker_queue_processor():
    while True:
        progs = await get_pending_programs(db_pool, limit=5)
        if not progs:
            await asyncio.sleep(60)
            continue
            
        # Si devuelve un solo diccionario (limit=1 internamente) lo convertimos a lista
        if isinstance(progs, dict) or not isinstance(progs, list):
            progs = [progs]
            
        logger.info(f"Iniciando subida de lote de {len(progs)} programas a Telegram...")
        
        for prog in progs:
            await update_status(db_pool, prog['id'], 'Procesando')
            await process_single_program(prog)
            
            # Pequeña pausa entre subidas del mismo lote para no saturar Telegram
            await asyncio.sleep(30)
            
        logger.info("Lote de programas enviado con exito. Hibernando 1 hora hasta el proximo lote...")
