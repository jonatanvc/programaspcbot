...
async def scheduled_scraper_task():
    while True:
        try:
            ...
            resultados.extend(await scrape_filecr())
    ...
    if len(file_ids) == len(part_paths):
        ...
        if PUBLIC_CHANNEL_ID != 0:
            async with publish_lock:
                texto = f"🌟 {prog['titulo']}\n\n"
                texto += f"🎲 Categoría: #{prog['categoria'].replace(' ', '_').replace('/', '_')}\n"
                texto += f"📅 Lanzamiento: Reciente\n"
                texto += f"👨‍💻 Soporte: Arquitectura Estable\n\n"
                texto += f"Descripción:\n{prog.get('descripcion', 'Utilidad distribuida para Windows.')[:300]}...\n\n"
                texto += f"💾 Tamaño: {size_str}   📚 Partes: {archivos_count}\n\n"
                texto += f"⬇️ Pulsa el botón inferior para abrir el asistente de descarga segura en el Bot."
