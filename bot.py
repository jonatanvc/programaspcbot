            ...
            if success:
                ...
                if requested_by:
                    try:
                        if request_msg_id:
                            try:
                                await application.bot.delete_message(chat_id=requested_by, message_id=request_msg_id)
                            except Exception as delete_e:
                                logger.error(f"Error eliminando mensaje anterior (Error): {delete_e}")
                    ...
                    try:
                        ...
                        if request_msg_id:
                            try:
                                ...
                                logger.error(f"No se pudo eliminar el mensaje anterior de pedido: {e}")
...
async def _button_callback_impl(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "menu_main":
        ...
        if item['type'] == 'db':
            ...
            prog = await get_program_by_id(db_pool, prog_id)
            if prog['estado'] in ['Pendiente', 'Error']:
                await db_pool.execute("UPDATE programas SET estado = 'Prioridad', requested_by = $1 WHERE id = $2;", user_id, prog_id)
                titulo = prog['titulo']
            elif prog['estado'] == 'Publicado':
            ...
            await insert_program(
                ...
                request_msg_id=query.message.message_id
            )
