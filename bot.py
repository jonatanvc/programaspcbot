    ...
    if len(file_ids) == len(part_paths):
        ...
        if PUBLIC_CHANNEL_ID != 0:
            async with publish_lock:
                ...
                # Obtener la fecha en un formato más bonito si es YYYY-MM-DD
                from datetime import datetime
                try:
                    dt = datetime.strptime(fecha_str, "%Y-%m-%d")
                    fecha_str = dt.strftime("%B %d, %Y")
                except:
                    pass

                texto = f"*Product Information*\n\n"
                texto += f"*File Name:* {prog['titulo']}\n"
                texto += f"*Created By:* {pub_str}\n"
                texto += f"*Version:* {version_str}\n"
                texto += f"*Release Date:* {fecha_str}\n"
                texto += f"*Languages:* {lang_str}\n\n"
                texto += f"*Categories:* {cat_str}\n\n"
                
                desc = prog.get('descripcion', 'Utilidad distribuida para Windows.')
                if len(desc) > 250:
                    desc = desc[:247] + "..."
                texto += f"{desc}\n\n"
                texto += f"💾 {size_str} | 📚 {archivos_count} Archivos"
