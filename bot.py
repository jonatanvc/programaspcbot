...
async def process_single_program(prog):
    async with download_semaphore:
        ...
        prog['requisitos'] = data.get('requisitos', prog.get('requisitos', ''))
        
        from database import update_program_metadata, delete_program
        try:
            await update_program_metadata(db_pool, prog['id'], prog['version'], prog['categoria'], prog['descripcion'], prog['imagen_url'], prog.get('titulo'), prog['fecha_actualizacion'], prog['requisitos'])
        except Exception as e:
        ...
        try:
            ...
            for i, fid in enumerate(file_ids, 1):
                try:
                    ...
                    if i == 1 and task.get('requisitos'):
...
async def message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Intentar eliminar el mensaje del usuario siempre
    try:
        await update.message.delete()
    except: pass
    ...
    if user_states.get(user_id) == "waiting_search":
        ...
        if not resultados:
            keyboard = InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Volver al Menú", callback_data="menu_main")]])
            error_text = "❌ <b>¡No se encontraron coincidencias!</b> 🗄️\n\n<code>Intente con otros términos de búsqueda.</code>"
            if last_msg_id:
                try:
        ...
        success_text = "🔍 <b>Coincidencias en nuestra Base de Datos:</b> 🗄️\n\n<code>Seleccione un programa de la lista:</code>"
        if last_msg_id:
            try:
        ...
        await context.bot.send_message(chat_id=user_id, text=success_text, reply_markup=InlineKeyboardMarkup(keyboard), parse_mode='HTML')
