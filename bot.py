    ...
    while True:
        try:
            ...
            if scheduled_scraper_task.caza_isos_mode:
                # MODO CAZA-ISOs
                from scraper import scrape_isos
                resultados.extend(await scrape_isos())
                scheduled_scraper_task.caza_isos_mode = False
            else:
                # MODO NORMAL
...
async def worker_queue_processor():
    ...
    while True:
        ...
        # Horario activo: 8:00 AM a 10:00 PM
        if now.hour < 8 or now.hour >= 22:
            # logger.debug(f"Fuera de horario (Actual: {now.strftime('%I:%M %p')}). Pausando subidas hasta las 8:00 AM...")
