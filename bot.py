...
async def process_single_program(prog):
    ...
    if len(file_ids) == len(part_paths):
        ...
        async with publish_lock:
            ...
            desc = prog.get('descripcion', 'Utilidad distribuida para Windows.').strip()
            if len(desc) > 350:
                # Buscar el último punto antes del límite para no dejarla incompleta
                last_period = desc.rfind('.', 0, 347)
                if last_period > 100:
...
async def show_main_menu(update: Update, context: ContextTypes.DEFAULT_TYPE, welcome_text=None):
    ...
    if not lang.startswith('es') and not welcome_text:
        try:
            ...
            btn_profile = await asyncio.to_thread(t.translate, btn_profile)
    ...
    keyboard = [
        ...
        [InlineKeyboardButton(btn_profile, callback_data="my_profile"), InlineKeyboardButton(btn_top_users, callback_data="top_users")],
...
async def _button_callback_impl(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "menu_main":
        ...
        cats = await get_categories(db_pool)
        keyboard = []
        # Agrupar en 2 columnas
        for i in range(0, len(cats), 2):
            row = []
            row.append(InlineKeyboardButton(f"📁 {cats[i]}", callback_data=f"cat_{cats[i]}"))
            if i + 1 < len(cats):
                row.append(InlineKeyboardButton(f"📁 {cats[i+1]}", callback_data=f"cat_{cats[i+1]}"))
            keyboard.append(row)
        ...
        if not profile:
            await query.edit_message_text("❌ <b>Perfil no encontrado.</b>", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Volver", callback_data="menu_main")]]), parse_mode='HTML')
            return
            
        fecha_reg = profile['fecha_registro'].strftime('%Y-%m-%d') if profile['fecha_registro'] else 'Desconocida'
        descargas = profile['descargas_count']
        
        texto = f"👤 <b>Tu Perfil de Usuario</b>\n\n"
        texto += f"🆔 <b>ID:</b> <code>{user_id}</code>\n"
        texto += f"📅 <b>Fecha de registro:</b> {fecha_reg}\n"
        texto += f"⬇️ <b>Programas descargados:</b> {descargas}\n\n"
        texto += f"<i>¡Sigue explorando nuestro catálogo para descubrir más programas!</i>"
...
async def message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if chat_type in ['group', 'supergroup']:
        ...
        if lower_text.startswith("#programa ") or lower_text.startswith("#os "):
            ...
            import re
            # Eliminar palabras de relleno usando límites de palabra (\b) para evitar dañar palabras reales (ej. 'telegram' -> 'tegram')
            termino = re.sub(r'\b(descargar|quiero|necesito|programa|buscar|por favor|el|la|un|una)\b', '', termino).strip()
...
async def inline_query_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    from database import search_programs_inline
    
    try:
        results = await search_programs_inline(db_pool, query, limit=40)
        ...
        for p in results:
            ...
            texto_mensaje = f"🤖 <b>{titulo}</b>\n\n📂 <b>Categoría:</b> {cat}\n📝 <b>Descripción:</b> {desc}\n\n⬇️ <b>Descárgalo usando nuestro bot oficial:</b>"
            
            keyboard = InlineKeyboardMarkup([[
                InlineKeyboardButton("🚀 Descargar Ahora", url=f"https://t.me/{context.bot.username}?start={p['id']}")
            ]])
            ...
            article = InlineQueryResultArticle(
                ...
                title=titulo,
                description=desc,
                thumbnail_url=p.get('imagen_url') or "https://i.imgur.com/3Z6YQ9H.png",
                input_message_content=InputTextMessageContent(texto_mensaje, parse_mode='HTML'),
                reply_markup=keyboard
            )
