        ...
        try:
            ...
            for prog in resultados:
                if not await program_exists(db_pool, prog['titulo'], prog['version']):
                    await insert_program(db_pool, prog['titulo'], prog['version'], prog.get('categoria', 'Software'), prog['url_origen'], prog.get('descripcion', ''))
                    nuevos += 1
    ...
    while True:
        ...
        if len(file_ids) == len(part_paths):
            ...
            if PUBLIC_CHANNEL_ID != 0:
                texto = f"🌟 **{prog['titulo']} {prog['version']}**\n\n"
                texto += f"💻 **Categoría:** {prog['categoria']}\n"
                texto += f"📝 **Descripción:** {prog.get('descripcion', 'Excelente utilidad para Windows.')[:300]}...\n\n"
                texto += f"⬇️ **Da clic al botón abajo para descargarlo de forma segura.**"
                ...
                import urllib.parse
                safe_title = urllib.parse.quote(prog['titulo'].split(' ')[0]) # Toma la primera palabra (nombre del programa)
                # Generar un icono profesional (Avatar) con la letra del programa
                image_url = f"https://ui-avatars.com/api/?name={safe_title}&background=random&color=fff&size=256&font-size=0.4"
        ...
        # Gotero: 5 programas por hora (1 cada 12 minutos)
        if len(file_ids) == len(part_paths):
            logger.info("Esperando 12 minutos para subir el siguiente programa (Modo Gotero)...")
...
async def start_command_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if args and args[0].isdigit():
        ...
        if prog['estado'] != 'Publicado' or not prog['telegram_file_ids']:
            await update.message.reply_text(f"⏳ **{prog['titulo']}** todavía está en la cola de descarga y no se ha subido a Telegram. ¡Vuelve a intentarlo más tarde!", parse_mode='Markdown')
            return
...
async def message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_states.get(user_id) == "waiting_search":
        ...
        for r in resultados:
            ...
            keyboard.append([InlineKeyboardButton(f"{estado_icon} {r['titulo']} ({r['categoria']})", url=f"https://t.me/{application.bot.username}?start={r['id']}")])
