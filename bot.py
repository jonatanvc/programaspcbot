...
async def worker_queue_processor():
    ...
    while True:
        ...
        # Queremos exactamente 3 por hora = 1 cada 20 minutos (1200 segundos) para permitir subidas de archivos muy grandes y priorizar el descanso del VPS
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "menu_main":
        ...
        if item:
            ...
            await query.edit_message_text(f"✅ <b>¡Programa Encontrado!</b>\n\n<code>Hemos pausado las subidas regulares para priorizar tu pedido: '{item['titulo']}'. Te enviaremos un mensaje automático en cuanto esté listo para descargar.</code>", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Volver al Menú", callback_data="menu_main")]]), parse_mode='HTML')
        else:
    ...
    if chat_type in ['group', 'supergroup']:
        ...
        if lower_text.startswith("#programa ") or lower_text.startswith("#os "):
            ...
            if item:
                ...
                await msg.edit_text(f"✅ <b>¡Pedido Registrado!</b>\n\n<code>Hemos encontrado '{item['titulo']}' y lo hemos puesto en la cola de prioridad. Te avisaré aquí mismo cuando esté subido.</code>", parse_mode='HTML')
            else:
                await insert_peticion(db_pool, user_id, termino_original)
