...
async def start_command_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    msg_not_found = "❌ Este programa no existe en la base de datos."
    msg_processing = "⏳ *{titulo}* todavía está en proceso de subida. ¡Prueba en unos minutos!"
    msg_queued = "✅ Descarga encolada. El bot te enviará los archivos comprimidos automáticamente en breves instantes..."
    msg_welcome = "🤖 Bienvenido. Utiliza los botones de descarga de nuestro canal."
    ...
    if not lang.startswith('es'):
        try:
            ...
            msg_not_found = t.translate(msg_not_found)
            msg_processing = t.translate(msg_processing.replace("*{titulo}*", "{titulo}")).replace("{titulo}", "*{titulo}*")
            msg_queued = t.translate(msg_queued)
            msg_welcome = t.translate(msg_welcome)
        except:
            pass
    ...
    if args and args[0].isdigit():
        ...
        if not prog:
            await update.message.reply_text(msg_not_found)
            return
            
        if prog['estado'] != 'Publicado' or not prog['telegram_file_ids']:
            await update.message.reply_text(msg_processing.format(titulo=prog['titulo']), parse_mode='Markdown')
            return
            
        import json
        file_ids_list = json.loads(prog['telegram_file_ids'])
        await update.message.reply_text(msg_queued)
...
async def show_main_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [
        ...
        [InlineKeyboardButton(btn_latest, callback_data="latest_prog")],
    ]
...
async def stats_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    texto = f"📊 *Panel de Estadísticas*\n\n"
    texto += f"👥 *Usuarios Totales:* {users}\n"
    texto += f"📦 *Programas Totales:* {total_progs} ({published} Publicados)\n\n"
    texto += f"🖥️ *Servidor (VPS):*\n"
    texto += f"CPU: {cpu}%\n"
    texto += f"RAM: {ram}%\n"
    texto += f"Disco: {disk}%\n"
...
async def broadcast_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(f"✅ Broadcast finalizado. Entregado a {count} usuarios.")
