    ...
    if len(file_ids) == len(part_paths):
        ...
        async with publish_lock:
            ...
            try:
                from deep_translator import GoogleTranslator
                t = GoogleTranslator(source='auto', target='es')
                desc = await asyncio.to_thread(t.translate, desc)
            except Exception as e:
...
async def worker_queue_processor():
    ...
    while True:
        ...
        if is_priority:
            ...
            sleep_time = max(10, 1200 - elapsed)
        
        if success:
            if is_priority:
                logger.info(f"Programa prioritario enviado en {elapsed/60:.1f} min. Omitiendo delay, siguiente en 10s...")
            else:
                logger.info(f"Programa enviado con éxito en {elapsed/60:.1f} min. Esperando {sleep_time/60:.1f} min para mantener el goteo exacto de 3 por hora...")
            
            # Notificar si fue un pedido prioritario
            requested_by = prog.get('requested_by')
            request_msg_id = prog.get('request_msg_id')
            
            if requested_by:
                try:
                    is_group = requested_by < 0
                    if is_group and isinstance(success, int): # success is channel_msg_id
                        post_link = f"https://t.me/c/{str(PUBLIC_CHANNEL_ID).replace('-100', '')}/{success}"
                        text_msg = f"🎉 <b>¡Tu pedido ya está listo!</b>\n\nEl programa <b>{prog['titulo']}</b> ha sido subido con éxito al canal."
                        keyboard = InlineKeyboardMarkup([[
                            InlineKeyboardButton("➡️ Ir al Post Solicitado", url=post_link)
                        ]])
                        ...
                        logger.info(f"Notificación de grupo enviada a {requested_by} para el programa {prog['id']}")
                    else:
                        await application.bot.send_message(
                            chat_id=requested_by,
                        ...
                        logger.info(f"Notificación privada enviada al usuario {requested_by} para el programa {prog['id']}")
                except Exception as e:
                    logger.error(f"No se pudo enviar notificación de prioridad al chat {requested_by}: {e}")
...
async def show_main_menu(update: Update, context: ContextTypes.DEFAULT_TYPE, welcome_text=None):
    ...
    if not lang.startswith('es') and not welcome_text:
        try:
            from deep_translator import GoogleTranslator
            t = GoogleTranslator(source='es', target=lang)
            texto = await asyncio.to_thread(t.translate, "Menú Principal del Gestor de Software")
            texto = f"🤖 <b>{texto}</b>"
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "menu_main":
        ...
        from scraper import scrape_search, scrape_search_getintopc
        item = await scrape_search(termino, require_os=require_os)
        if not item:
            item = await scrape_search_getintopc(termino, require_os=require_os)
        
        from database import insert_peticion, insert_program
        if item:
            # Enviar a cola de prioridad
            await insert_program(
                db_pool, 
                item['titulo'], 
                item['version'], 
            ...
            if cola <= 1:
                text_msg = f"✅ <b>¡Programa Encontrado!</b>\n\n<code>Hemos pausado todo para priorizar tu pedido: '{item['titulo']}'. Lo estamos descargando y subiendo AHORA MISMO. Te enviaremos el link cuando esté listo.</code>"
            else:
                nombres_cola = await get_priority_queue_names(db_pool, 3)
                lista_nombres = "\n".join([f"• {n}" for n in nombres_cola])
                
                text_msg = f"✅ <b>¡Programa Encontrado!</b>\n\n<code>Hemos registrado tu pedido: '{item['titulo']}'. Estás en la posición #{cola} de la cola prioritaria. Te enviaremos el link en cuanto esté listo.</code>\n\n<b>⏳ Procesando actualmente:</b>\n<i>{lista_nombres}</i>"
            ...
            await insert_peticion(db_pool, user_id, termino_original)
            await query.edit_message_text(f"⚠️ <b>No encontrado automáticamente</b>\n\n<code>Lamentablemente no encontramos '{termino_original}' en nuestras fuentes en este momento. Lo hemos guardado en nuestra lista de deseos para revisión manual.</code>", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Volver al Menú", callback_data="menu_main")]]), parse_mode='HTML')
...
async def message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if chat_type in ['group', 'supergroup']:
        ...
        if lower_text.startswith("#programa ") or lower_text.startswith("#os "):
            ...
            from database import insert_peticion, insert_program, count_priority_queue
            if item:
                await insert_program(
                    db_pool, 
                    item['titulo'], 
                    ...
                    item['fecha_actualizacion'], 
                    estado='Prioridad',
                    requested_by=update.message.chat.id,
                ...
                cola = await count_priority_queue(db_pool)
                if cola <= 1:
                    text_msg = f"✅ <b>¡Pedido Registrado!</b>\n\n<code>Hemos encontrado '{item['titulo']}'. Lo estamos descargando y procesando AHORA MISMO. Te avisaré aquí cuando esté subido.</code>"
                else:
                    nombres_cola = await get_priority_queue_names(db_pool, 3)
                    lista_nombres = "\n".join([f"• {n}" for n in nombres_cola])
                    text_msg = f"✅ <b>¡Pedido Registrado!</b>\n\n<code>Hemos encontrado '{item['titulo']}'. Estás en la posición #{cola} de la cola prioritaria. Te avisaré aquí cuando esté subido.</code>\n\n<b>⏳ Procesando actualmente:</b>\n<i>{lista_nombres}</i>"
                    
                await msg.edit_text(text_msg, parse_mode='HTML')
            else:
                await insert_peticion(db_pool, user_id, termino_original)
                await msg.edit_text(f"⚠️ <b>No encontrado automáticamente</b>\n\n<code>Lo he guardado en nuestra lista de deseos para que un administrador lo revise manualmente.</code>", parse_mode='HTML')
        return
