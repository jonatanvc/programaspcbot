...
async def user_queue_processor():
    while True:
        try:
            ...
            try:
                msg_pass = (
                    "🔑 <b>Contraseña para extraer:</b> <code>123</code>\n"
                    "⚠️ <i>Recuerde desactivar su Antivirus/Windows Defender temporalmente "
                    "antes de extraer si el software viene pre-activado (crack/patch).</i>"
                )
                await application.bot.send_message(chat_id=user_id, text=msg_pass, parse_mode='HTML')
            except: pass
