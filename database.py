...
async def count_priority_queue(pool):
    query = "SELECT COUNT(*) FROM programas WHERE estado = 'Prioridad' AND requested_by IS NOT NULL;"
    async with pool.acquire() as conn:
        result = await conn.fetchval(query)
        return result or 0

async def get_priority_queue_names(pool, limit=3):
    query = f"SELECT titulo FROM programas WHERE estado = 'Prioridad' AND requested_by IS NOT NULL ORDER BY id ASC LIMIT {int(limit)};"
    async with pool.acquire() as conn:
        records = await conn.fetch(query)
