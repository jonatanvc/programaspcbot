...
async def init_db(pool):
    ...
    async with pool.acquire() as conn:
        ...
        try:
            await conn.execute("ALTER TABLE usuarios ADD COLUMN IF NOT EXISTS descargas_count INT DEFAULT 0;")
            ...
            await conn.execute("ALTER TABLE programas ADD COLUMN IF NOT EXISTS request_chat_id BIGINT;")
            await conn.execute("ALTER TABLE programas ADD COLUMN IF NOT EXISTS status_msg_id BIGINT;")
        except Exception:
            pass
...
async def get_priority_queue_names(pool, limit=3):
    query = f"SELECT titulo FROM programas WHERE estado IN ('Prioridad', 'Procesando') AND requested_by IS NOT NULL ORDER BY CASE WHEN estado = 'Procesando' THEN 1 ELSE 2 END, id ASC LIMIT {int(limit)};"
    async with pool.acquire() as conn:
        records = await conn.fetch(query)
