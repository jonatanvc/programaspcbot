...
async def insert_program(pool, titulo, version, categoria, url_origen, descripcion="", imagen_url="", fecha_actualizacion="Reciente"):
    query = """
    INSERT INTO programas (titulo, version, categoria, url_origen, descripcion, imagen_url, fecha_actualizacion)
    VALUES ($1, $2, $3, $4, $5, $6, $7)
    ON CONFLICT (titulo, version) DO NOTHING
    RETURNING id;
    """
    async with pool.acquire() as conn:
        result = await conn.fetchval(query, titulo, version, categoria, url_origen, descripcion, imagen_url, fecha_actualizacion)
        return result
...
async def search_programs(pool, search_query):
    query = "SELECT * FROM programas WHERE titulo ILIKE $1 ORDER BY id DESC LIMIT 10;"
    async with pool.acquire() as conn:
