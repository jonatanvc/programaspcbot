...
async def insert_program(pool, titulo, version, categoria, url_origen, descripcion="", imagen_url=""):
    query = """
    INSERT INTO programas (titulo, version, categoria, url_origen, descripcion, imagen_url)
    VALUES ($1, $2, $3, $4, $5, $6)
    ON CONFLICT (titulo, version) DO NOTHING
    RETURNING id;
    """
    async with pool.acquire() as conn:
        result = await conn.fetchval(query, titulo, version, categoria, url_origen, descripcion, imagen_url)
        return result
