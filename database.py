...
async def search_programs_inline(pool, query_text, limit=30):
    ...
    for i, word in enumerate(words, 1):
        conditions.append(f"titulo ILIKE ${i}")
        args.append(f'%{word}%')
    
    args.append(limit)
    where_clause = " AND ".join(conditions)
    query = f"SELECT id, titulo, categoria, descripcion, imagen_url FROM programas WHERE estado = 'Publicado' AND {where_clause} ORDER BY descargas_count DESC LIMIT ${len(args)};"
