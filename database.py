...
async def update_program_metadata(pool, program_id, version, categoria, descripcion, imagen_url, titulo=None, fecha_actualizacion='Reciente', requisitos=''):
    if titulo:
        query = """
        UPDATE programas
        SET version = $1, categoria = $2, descripcion = $3, imagen_url = $4, titulo = $6, fecha_actualizacion = $7, requisitos = $8
        WHERE id = $5;
        """
        async with pool.acquire() as conn:
            await conn.execute(query, version, categoria, descripcion, imagen_url, program_id, titulo, fecha_actualizacion, requisitos)
    else:
        query = """
        UPDATE programas
        SET version = $1, categoria = $2, descripcion = $3, imagen_url = $4, fecha_actualizacion = $6, requisitos = $7
        WHERE id = $5;
        """
        async with pool.acquire() as conn:
            await conn.execute(query, version, categoria, descripcion, imagen_url, program_id, fecha_actualizacion, requisitos)
